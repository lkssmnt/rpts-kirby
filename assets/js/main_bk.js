...
function createCollection(wrapperSelector) {
  ...
  allRemoveBtns.forEach(btn => {
    btn.addEventListener("click", (event) => {
      ...
      fetchThis = newCollection.map((collection) => {
        return [collection.slug, filterThis];
      });
    })
  });
  ...
  // put something in the front of the fetchThis array
  fetchThis.unshift(["/assets/html/collection-header.html", filterThis]);
}
...
async function printPreview(filterTag, contentToFetch) {
  ...
  if (contentToFetch) {
    for (var i = 0; i < contentToFetch.length; i++) {
      ...
      console.log(contentToFetch[i][0], contentToFetch[i][1]);
